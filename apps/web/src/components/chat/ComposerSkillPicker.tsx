...
export const ComposerSkillPicker = memo(function ComposerSkillPicker(props: {
  ...
  return (
    <Popover>
      ...
      <PopoverPopup align="start" side="top" className="w-84 p-0">
        <div className="border-b border-border/60 px-4 py-3">
          <div className="font-medium text-foreground text-sm">Skills</div>
          <div className="mt-1 text-muted-foreground text-xs leading-relaxed">
            Attach reusable instructions from `./agents/skills` or `~/.agents/skills`.
          </div>
        </div>
        <div className="max-h-80 overflow-y-auto px-2 py-2">
          {props.skills.length === 0 ? (
            <div className="rounded-lg border border-dashed border-border/70 bg-muted/15 px-3 py-3 text-muted-foreground text-sm">
              No skills found.
            </div>
          ) : (
