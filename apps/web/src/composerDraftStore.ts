    ...
    (setBase, get) => {
      ...
      return {
        ...
        appendMentionToPrompt: (threadId, relativePath) => {
          ...
          const state = get();
          const currentPrompt = state.draftsByThreadKey[threadId]?.prompt ?? "";
          const mention = `@${relativePath}`;
          const separator = currentPrompt.length > 0 && !currentPrompt.endsWith(" ") ? " " : "";
          state.setPrompt(DraftId.makeUnsafe(threadId), `${currentPrompt}${separator}${mention} `);
        },
        setSelectedSkills: (threadId, skillNames) => {
          ...
          set((state) => {
            const existing = state.draftsByThreadKey[threadId];
            ...
            const unchanged =
              ...
              base.selectedSkills.every(
                (skillName, index) => skillName === normalizedSkills[index],
              );
            if (unchanged) {
              return state;
            }
            ...
            const nextDraftsByThreadKey = { ...state.draftsByThreadKey };
            if (shouldRemoveDraft(nextDraft)) {
              delete nextDraftsByThreadKey[threadId];
            } else {
              nextDraftsByThreadKey[threadId] = nextDraft;
            }
            return { draftsByThreadKey: nextDraftsByThreadKey };
          });
        },
